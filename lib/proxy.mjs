// dsh-pocket 核心：Host/Origin 改写反向代理 + Gitee OAuth 访问控制
//
...
// 都不需要改 dsh 的任何配置。
//
// 认证模型（Gitee OAuth，代替旧 PIN）：
//   - loopback（本机）免认证；
//   - 其余 Host 一律要求会话 cookie（HttpOnly，值 = sha256(boundUid:sessionKey)）；
//   - 会话经 /pocket-oauth/start → gitee 授权 → /pocket-oauth/callback（uid 与本机
//     绑定的 boundUid 比对，一致才种 cookie）获得；
//   - /pocket-setup 仅 loopback 可达：填 Client ID/Secret 与回调 origin 白名单并绑定账号。
//
...
<h1>🖥️ DSH Pocket</h1>
<p>你已通过 Gitee 登录，但页面仍被拦截。<br>因为本机 DSH Desktop 未开启「浏览器访问」，桌面门禁拒绝了普通浏览器（含手机）的页面请求。</p>
<div class="step">
...
<p style="margin-top:14px">注意：Gitee 登录成功 ≠ 已获得桌面 Web 访问授权。门禁由 DSH Desktop 控制，pocket 无法代为绕过。</p>
...
/** 登录页：Gitee OAuth 入口。 */
function oauthLoginPageHtml() {
  return pageHtml('DSH Pocket · 登录', `
<h1>🔐 DSH Pocket</h1>
<p>本地址受 Gitee 登录保护<br>This address is protected by Gitee sign-in</p>
<a class="btn" href="/pocket-oauth/start">使用 Gitee 账号登录<br>Sign in with Gitee</a>
<p class="muted" style="margin-top:12px">只有在本机绑定的 Gitee 账号可以进入——就是创建 OAuth 应用的那个账号，全程只需一个账号<br>Only the Gitee account bound on this machine may enter — the very account that owns the OAuth app; one account is all you need</p>`);
}

/** 未初始化提示页（fail closed 的可操作出口）。 */
function unconfiguredPageHtml(setupUrl) {
  return pageHtml('DSH Pocket · 尚未初始化', `
<h1>⚙️ DSH Pocket</h1>
<p>本机还没有完成 Gitee OAuth 初始化，所有远程访问均已关闭（fail closed）。<br>This machine has not finished Gitee OAuth setup; all remote access is closed.</p>
<ul>
<li>① 在 Gitee 创建 OAuth 应用：<code>https://gitee.com/oauth/applications</code>，回调地址填 <code>${OAUTH_CALLBACK_PATH}</code> 结尾的完整 URL（可注册多个）</li>
<li>② 在<strong>本机</strong>浏览器打开 <code>${escapeHtml(setupUrl)}</code> 填入 Client ID / Secret 并绑定账号</li>
</ul>`);
}

/** 已存凭据但未绑定：继续在 setup 页点「绑定」。 */
function unboundPageHtml(setupUrl) {
  return pageHtml('DSH Pocket · 待绑定账号', `
<h1>🔗 DSH Pocket</h1>
<p>Client 凭据已保存，但还没有绑定 Gitee 账号。<br>Credentials saved, but no Gitee account is bound yet.</p>
<p>在本机浏览器打开 <code>${escapeHtml(setupUrl)}</code>，点击「保存并绑定 Gitee 账号」完成最后一步（用创建应用时的<strong>同一个</strong> Gitee 账号授权）。<br>Open it on this machine and click "Save &amp; bind Gitee account" — authorize with the same account that owns the OAuth app.</p>`);
}

/** 白名单外 origin 的引导页。 */
function originNotAllowedPageHtml(allowed) {
  const list = (allowed ?? []).map((o) => `<li><code>${escapeHtml(o)}</code></li>`).join('');
  return pageHtml('DSH Pocket · 请使用配置的访问地址', `
<h1>🧭 DSH Pocket</h1>
<p>当前访问的地址不在回调白名单里，无法发起 Gitee 登录。<br>This origin is not in the callback allowlist.</p>
<p>请改用以下地址访问（或在 Gitee 应用与本机 /pocket-setup 中同时登记当前地址）：</p>
...
/** Gitee 侧返回错误（用户拒绝授权等）。 */
function oauthDeniedPageHtml(detail) {
  return pageHtml('DSH Pocket · 授权未完成', `
<h1>🚫 DSH Pocket</h1>
<p>Gitee 授权未完成${detail ? `：<code>${escapeHtml(detail)}</code>` : ''}<br>Gitee authorization did not complete.</p>
<a class="btn" href="/pocket-oauth/start">重新登录 | Retry</a>`);
}

/** 账号不匹配：不是本机绑定的 Gitee 账号。 */
function oauthMismatchPageHtml(login) {
  return pageHtml('DSH Pocket · 账号未绑定', `
<h1>⛔ DSH Pocket</h1>
<p>当前登录的 Gitee 账号 <strong>${escapeHtml(login || '未知')}</strong> 未绑定本机，访问被拒绝。<br>This Gitee account is not bound to this machine.</p>
<p>如需换绑，请在<strong>本机</strong>打开 <code>/pocket-setup</code> 重新绑定。</p>`);
}

/** 换票/取用户失败等内部错误。 */
function oauthErrorPageHtml(message) {
...
<p class="muted">若持续失败，请检查网络能否访问 gitee.com，以及 Client ID / Secret / 回调地址是否与 Gitee 应用一致。</p>`);
}

/** 绑定成功页。 */
function bindSuccessPageHtml(login) {
  return pageHtml('DSH Pocket · 绑定成功', `
<h1>✅ 绑定成功</h1>
<p>已绑定 Gitee 账号 <strong>${escapeHtml(login)}</strong>。<br>Bound to Gitee account <strong>${escapeHtml(login)}</strong>.</p>
 ...
 *
 * 注意：OAuth 回调（`/pocket-oauth/callback`）**不能**走这个检查——它本来就是 Gitee
...
<p class="muted">① 在 <code>gitee.com/oauth/applications</code> 创建应用，「应用回调地址」登记（可多条）：<code>&lt;访问地址&gt;${OAUTH_CALLBACK_PATH}</code><br>
例如 <code>http://127.0.0.1:${port}${OAUTH_CALLBACK_PATH}</code>（本机）、<code>https://你的固定域名${OAUTH_CALLBACK_PATH}</code>（隧道）、<code>http://局域网IP:${port}${OAUTH_CALLBACK_PATH}</code><br>
② 下方「访问地址」与 Gitee 的回调地址填<strong>相同内容</strong>即可——整条复制过来也行（结尾的 <code>${OAUTH_CALLBACK_PATH}</code> 会自动去掉）。保存后跳转 Gitee，用<strong>同一个</strong>账号授权完成绑定。</p>
...
<input name="client_secret" type="password" placeholder="${hasSecret ? '••••••••' : 'Gitee 应用的 Client Secret'}" ${hasSecret ? '' : 'required'}>
<label>访问地址（与 Gitee 应用回调地址填相同内容；每行一个，最多 10 条）</label>
...
<button type="submit">保存并绑定 Gitee 账号</button>
</form>
<p class="muted">两边填相同内容即可：直接把 Gitee 的回调地址整条复制过来（结尾的 <code>${OAUTH_CALLBACK_PATH}</code> 会自动去掉），或填「协议://主机[:端口]」；含协议与端口，须逐字符一致。</p>`);
}
 ...
 * @param {object} [opts.auth]       Gitee OAuth 认证配置：
 *   - sessionKey: string | (() => string) —— 进程级会话密钥（函数形式支持轮换＝登出所有设备）
 *   - getConfig(): OAuth 配置（每次请求实时读；见 lib/oauth.mjs readOAuthConfig）
 *   - saveConfig?(draft): setup 页保存凭据（可注入测试桩）
 *   - bindUser?(user): 绑定账号落盘（可注入测试桩）
 *   - giteeBase?/fetchImpl?/stateTtlMs?: 测试注入
...
/** setup 表单 nonce（进程级随机）：配合 isSameSiteRequest 拦跨站伪造提交。 */
  ...
  const giteeBase = auth?.giteeBase ?? DEFAULT_GITEE_BASE;
  ...
  async function handleOAuthRoute(req, res, pathname, loopback) {
    ...
    if (pathname === '/pocket-oauth/start') {
      // 来源校验先于一切：跨站页面不得代办登录/绑定（发起登录本身也会写 state 并跳 Gitee）
...
<p>绑定账号是初始化操作，只能在本机（127.0.0.1）进行。<br>Binding may only be initiated from this machine (loopback).</p>`));
      ...
      if (!origin) {
        sendHtml(res, 403, originNotAllowedPageHtml(cfg.callbackOrigins));
        return true;
      }
      if (purpose === 'login' && !cfg.boundUid) {
        sendHtml(res, 503, unboundPageHtml(setupUrl()));
        return true;
      }
      const state = states.create(purpose);
      res.writeHead(302, { location: giteeAuthorizeUrl({ clientId: cfg.clientId, redirectUri: `${origin}${OAUTH_CALLBACK_PATH}`, state, baseUrl: giteeBase }), 'cache-control': 'no-store' });
      res.end();
      return true;
    }
    ...
    if (pathname === OAUTH_CALLBACK_PATH) {
      ...
      if (errParam) {
        sendHtml(res, 403, oauthDeniedPageHtml(`${errParam}${u.searchParams.get('error_description') ? `: ${u.searchParams.get('error_description')}` : ''}`));
        return true;
      }
      const code = u.searchParams.get('code');
      const purpose = states.consume(u.searchParams.get('state'));
      if (!code || !purpose) {
      ...
      let user;
      try {
        user = await exchangeCodeForUser({ clientId: cfg.clientId, clientSecret: cfg.clientSecret, code, redirectUri: `${origin}${OAUTH_CALLBACK_PATH}`, fetchImpl, baseUrl: giteeBase });
      } catch (err) {
        log?.(`dsh-pocket: oauth exchange failed | OAuth 换票失败: ${err?.message ?? err}`);
        sendHtml(res, 502, oauthErrorPageHtml(err?.message ?? String(err)));
        return true;
      }
      if (purpose === 'bind') {
        try {
          await auth.bindUser?.(user);
        } catch (err) {
          log?.(`dsh-pocket: bind failed | 绑定失败: ${err?.message ?? err}`);
          sendHtml(res, 500, oauthErrorPageHtml(err?.message ?? String(err)));
          return true;
        }
        sendHtml(res, 200, bindSuccessPageHtml(user.login));
        return true;
      }
      if (String(user.id) !== String(cfg.boundUid)) {
        log?.(`dsh-pocket: uid mismatch (${user.login}) | 账号不匹配`);
        sendHtml(res, 403, oauthMismatchPageHtml(user.login));
        return true;
      }
...
<p>初始化页面只能在本机（127.0.0.1）打开。<br>The setup page is loopback-only.</p>`));
    ...
    if (pathname === '/pocket-setup/save') {
      ...
      if (!clientId || !clientSecret) {
        sendHtml(res, 400, setupHtml({ ...prev, clientId }, 'Client ID / Secret 不能为空 | Client ID / Secret required'));
        return true;
      }
      if (originsResult.error) {
        sendHtml(res, 400, setupHtml({ ...prev, clientId }, originsResult.error));
        return true;
      }
      try {
        ...
        sendHtml(res, 500, setupHtml({ ...prev, clientId }, err?.message ?? String(err)));
        return true;
      }
    ...
    if (auth && !loopback) {
      ...
      if (!sessionOk(req, cfg)) {
        if (!cfg?.clientId || !cfg?.clientSecret) {
          ...
          if (isHtmlRequest(req)) sendHtml(res, 503, unboundPageHtml(`http://127.0.0.1:${listeningPort || 3081}/pocket-setup`));
          else sendJson(res, 503, { error: 'oauth-not-bound' });
        } else if (isHtmlRequest(req)) {
          sendHtml(res, 200, oauthLoginPageHtml());
        } else {
