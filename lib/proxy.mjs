...
<p class="muted" style="margin-top:12px">只有在本机绑定的 Gitee 账号可以进入<br>Only the Gitee account bound on this machine may enter</p>`);
}
...
<p>在本机浏览器打开 <code>${escapeHtml(setupUrl)}</code>，点击「保存并绑定 Gitee 账号」完成最后一步。</p>`);
}
...
function setupPageHtml(cfg, error, port = 3081) {
...
② 填入下方信息并保存，随后跳转到 Gitee 完成账号绑定。</p>
...
<label>访问地址（回调 origin 白名单，每行一个，最多 10 条）</label>
...
<p class="muted">Gitee 的回调地址与这里的每行地址 + <code>${OAUTH_CALLBACK_PATH}</code> 必须逐字符一致（含协议与端口）。</p>`);
}
...
export function createPocketProxy({
  ...
  async function handleOAuthRoute(req, res, pathname, loopback) {
    ...
    /** setup 页渲染（示例地址用真实监听端口）。 */
    const setupHtml = (cfg, err) => setupPageHtml(cfg, err, listeningPort || 3081);
...
<p>绑定账号是初始化操作，只能在本机（127.0.0.1）进行。<br>Binding may only be initiated from this machine (loopback).</p>`));
    ...
    if (pathname === OAUTH_CALLBACK_PATH) {
      ...
      // 登录成功：种会话 cookie（HttpOnly；绑定进程级 sessionKey）
      res.writeHead(303, {
        location: '/',
        'set-cookie': `${SESSION_COOKIE}=${sessionCookieValue(cfg.boundUid, resolveSessionKey())}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${SESSION_MAX_AGE}`,
