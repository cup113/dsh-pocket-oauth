...
/**
 * 归一化单个回调 origin。两种写法都接受（用户经常把 Gitee 应用里登记的
 * 完整回调地址整条复制过来，不该为此报错）：
...
export function normalizeOrigin(input) {
  ...
  if ((u.username || u.password) || (!isCallbackUrl && path !== '/') || u.search || u.hash) {
    return { error: `回调地址两种写法都可以：「协议://主机[:端口]」，或 Gitee 里复制的完整回调地址（…${OAUTH_CALLBACK_PATH}）；不接受其他路径：${raw} | use scheme://host[:port], or the full callback URL copied from Gitee (…${OAUTH_CALLBACK_PATH}); other paths are rejected` };
  }
