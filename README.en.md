...
| 📱 QR per origin         | The settings tab renders a QR code for every allowlisted origin, grouped into "this machine / LAN" and "public (self-hosted tunnel)" with each group's prerequisites; multiple origins in a group sit side by side in columns (single column on narrow screens) |
