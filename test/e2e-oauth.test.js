...
// 唯一被替换的是外部 Gitee 端点（网络边界），其余全部走生产代码路径。
//
// 覆盖用户旅程：
//   未初始化 fail closed → 本机 setup 保存凭据 → 本机完成 Gitee 绑定（写盘）
...
/** 桩 Gitee：token 交换 + 用户信息（uid/login 可切换以模拟不同账号）。 */
...
/** 起真实插件入口（只桩外部 Gitee + 网络边界 stub 掉桌面检测）。
 *  applyFn 由调用方给出：需要模拟「新进程」时用带缓存击穿的模块实例（见 loadEntry）。 */
async function startEntry({ upstreamPort, dshHome, gitee, applyFn = apply }) {
  ...
  applyFn(ctx, {}, {
    port: 0,
    oauthTestHooks: { giteeBase: 'http://gitee.test', fetchImpl: stubGitee(gitee) },
  });
  // 等代理就绪并拿到真实端口
  let port = null;
...
test('E2E：未初始化 fail closed → 本机绑定（写盘）→ 本机免登录 → 隧道域名同账号登录 → 换账号被拒', async () => {
...
test('E2E：dsh web 重启（新进程）后旧会话 cookie 失效，需重新 Gitee 登录', async () => {
  const up = await fakeUpstream();
